...
public class LoginController {
    ...
    private int getUserType(String login) {
        EntityManager em = ENTITY_MANAGER_FACTORY.createEntityManager();
        String query = "SELECT w.usertype FROM EmployeeEntity w WHERE w.login=:login";
    ...
    private String getUserPassword(String login) {
        EntityManager em = ENTITY_MANAGER_FACTORY.createEntityManager();
        String query = "SELECT w.password FROM EmployeeEntity w WHERE w.login=:login";
        Query q = em.createQuery(query);
        q.setParameter("login", login);
        String result = null;
        try {
    ...
    private void addLoginLog(String login) {
        ...
        TypedQuery<EmployeeEntity> tq = getWorker(em, login);
        ...
        try {
            ...
            log.setEmployeeByIdWorker(tq.getSingleResult());
    ...
    private TypedQuery<EmployeeEntity> getWorker(EntityManager em, String login) {
        return em.createQuery("SELECT w FROM EmployeeEntity w WHERE w.login=:login", EmployeeEntity.class).setParameter("login", login);
    }
}