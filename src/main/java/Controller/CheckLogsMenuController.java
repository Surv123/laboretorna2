...
public class CheckLogsMenuController implements Initializable {
    ...
    private List<String> getWorkerList(){
        ...
        Query q = em.createQuery("SELECT w FROM EmployeeEntity w WHERE w.usertype <> 1");
    ...
    private void updateTable(String f, String i){
        ...
        Query q = em.createQuery("SELECT l FROM LogEntity l " +
                "INNER JOIN EmployeeEntity w on l.employeeByIdWorker.idWorker = w.idWorker " +
                "WHERE w.surnameW=:f and  w.nameW=:i").setParameter("f", f).setParameter("i", i);
